...
import isPromise from 'relay-runtime/lib/isPromise';
...
import type {CacheConfig, Disposable} from 'relay-runtime/lib/RelayCombinedEnvironmentTypes';
import type {ConcreteBatch} from 'relay-runtime/lib/RelayConcreteNode';
import type {
  FetchFunction,
  Network,
  QueryPayload,
...
import type {Observer} from 'relay-runtime/lib/RelayStoreTypes';
import type {Variables} from 'relay-runtime/lib/RelayTypes';

/**
 * Creates an implementation of the `Network` interface defined in
 * `RelayNetworkTypes` given a single `fetch` function.
 */
function create(callback, fetch: FetchFunction, subscribe?: SubscribeFunction): Network {
  function request(
  ...
  function requestStream(
    ...
    const onRequestError = error => {
      ...
      if (error && error.message == 'Err-11111: authentication failed') {
        callback(error);
        return;
      }
